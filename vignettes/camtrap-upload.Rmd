...
As of `weda v0.0.1` you can upload data to the database using an interactive shiny app. You can also view previously uploaded records on a map and download presence-absences. To run the app from `R` set up a database connection (using `weda_connect()`) and then run the app using `camtrap_app()`:  
...
```{r upload, eval = FALSE}
con_odbc <- weda_connect(password = keyring::key_get(service = "ari-dev-weda-psql-01", 
                                                     username = "psql_user"))
